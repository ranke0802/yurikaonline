...
< ## 📋 최신 업데이트 내역 (0.00.45)
< 
< ### 0.00.45 - 보스 로직 및 밸런스 개선
< - **퀘스트 연동 로직**: 슬라임 퀘스트를 완료한 플레이어가 접속 시, 글로벌 처치 카운트가 최소 10으로 보정됩니다 (빠른 보스 소환 유도).
< - **자동 초기화**: 호스트가 없는 상태(비활성)가 1분 이상 지속되면 처치 카운트가 초기화됩니다.
< - **시각효과**: 앱솔루트 베리어(K) 사용 시 플레이어와 몬스터 모두에게 방패 아이콘(Shield Icon)이 표시됩니다.
< - **밸런스**: 플레이어의 매직 미사일(H) 사거리가 600으로 조정되었습니다.
---
> ## 📋 최신 업데이트 내역 (0.00.49)
>
> ### 0.00.49 - 퀘스트 및 보스 소환 로직 고도화
> - **처치 카운트 로직 개선**:
>   - **첫 진입 (퀘스트 1 완료)**: 슬라임 10마리 처치 퀘스트 완료 시, 대왕 슬라임 소환 카운트가 즉시 10/30으로 보정됩니다.
>   - **반복 (보스 처치 후)**: 대왕 슬라임 처치 시 소환 카운트가 0/30으로 초기화되며, 퀘스트도 0/1로 즉시 재시작됩니다.
> - **UI 편의성**: 대왕 슬라임 퀘스트 하단에 "소환 진행도: X/30"이 실시간으로 표시됩니다.
> - **감전(Slow) 효과 강화**: 체인 라이트닝의 감전 지속시간이 4초로 증가하고, 감전 시 몬스터의 돌진 속도도 50% 느려집니다.
> - **대왕 슬라임 AI**: 대왕 슬라임이 전투 중 주기적으로 앱솔루트 베리어(1.5초 완전 방어)를 사용합니다.
> - **보스 완료 모달**: 대왕 슬라임 처치 시 화려한 보상 획득 모달(EXP/Gold)이 출력됩니다.
>
> ### 0.00.45 ~ 0.00.46 - 보스 로직 및 밸런스 개선
> - **퀘스트 연동 로직 수정**: 반복 가능한 보스 퀘스트를 위해 기존의 단순 점프 로직은 폐기되고 카운트 기반으로 재설계되었습니다.
> - **자동 초기화**: 호스트 비활성(1분) 시 처치 카운트 초기화.
> - **시각효과**: 앱솔루트 베리어(K)가 피격 시 1.5초간 무적(Invincibility) 상태를 부여하며 아이콘이 표시됩니다.
> - **밸런스**: 매직 미사일 사거리 600, 대왕 슬라임 돌진 사거리가 800으로 조정되었습니다.
- **대왕 슬라임 소환 로직**:
  - 소환되어 있는 동안에는 슬라임 처치 수가 카운트되지 않도록 변경하여 소환 주기를 조절했습니다.
  - 처치 카운트가 DB에 저장되어 서버 재시작이나 호스트 변경 시에도 유지됩니다.
- **돌진(Charge) 스킬 추가**:
  - 모든 슬라임이 일정 거리 내에서 돌진 스킬을 사용합니다 (붉은색 범위 경고 후 돌진).
  - **일반 슬라임**: 사거리 400, 쿨타임 4초, 데미지 15.
  - **분열된 슬라임**: 사거리 500, 쿨타임 3초, 데미지 30.
  - **대왕 슬라임**: 사거리 600, 쿨타임 3초, 데미지 50.
