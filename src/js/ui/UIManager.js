export class UIManager {
    ...
    setupEventListeners() {
        ...
        this.skillData = {
            laser: { name: '체인 라이트닝 (J)', desc: '공격 유지 시 위력이 증폭되는 연쇄 번개를 방출합니다. 감전된 적은 속도가 둔화됩니다. [연쇄: 기본 2마리 / 1레벨당 +1] [적중 시 마나 회복]' },
    ...
    showTooltip(skillId, x, y) {
        ...
        switch (skillId) {
            case 'laser':
                ...
                const slow = Math.floor((0.3 + (lv - 1) * 0.03) * 100);
                currentEffect = `<div class="current-effect">현재 효과 (Lv.${lv}):<br>연쇄: ${baseChain}마리 | 위력: ${startDmg} ~ ${maxDmg} | 둔화: ${slow}%</div>`;
                break;
            case 'missile':
                const mCount = lv;
